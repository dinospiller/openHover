...
/**
 * Process one sample of a first-order filter
 * @param filter
 * The filter object that stores the memory of previous samples and coeffs
 * @param new_sample
...
/**
 * Reset all internal variables and setup coefficients
 */
void filter_init_first_order_filter(FILT_first_order *filt,float N0, float N1, float D1){
	filter_reset_first_order_filter(filt);
	filt->N0=N0;
	filt->N1=N1;
	filt->D1=D1;
}
...
void filter_init_second_order_filter(FILT_second_order *filt,float N0, float N1, float N2, float D1, float D2){
	filter_reset_second_order_filter(filt);
	filt->N0=N0;
	filt->N1=N1;
	filt->N2=N2;
	filt->D1=D1;
...
void filter_reset_second_order_filter(FILT_second_order *filt){
	filt->prev_input=0.0;
	filt->preprev_input=0.0;
	filt->prev_output=0.0;
	filt->preprev_output=0.0;
}
