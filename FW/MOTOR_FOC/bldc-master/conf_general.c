...
// EEPROM settings
#define EEPROM_BASE_MCCONF				100//1000
#define EEPROM_BASE_APPCONF				300//2000
#define EEPROM_BASE_SELF_BAL_CONF		600//3000
...
void conf_general_init(void) {
	...
	for (unsigned int i = 0;i < (sizeof(SELF_BALANCING_PARAMS) / 2);i++) {
		VirtAddVarTab[ind++] = EEPROM_BASE_SELF_BAL_CONF + i;
	}
...
/**
 * Load the compiled default self-balancing settings.
 *
 * @param conf
 * A pointer to store the default configuration to.
 */
void conf_general_get_default_self_balancing_params(SELF_BALANCING_PARAMS *conf) {
	memset(conf, 0, sizeof(SELF_BALANCING_PARAMS));

	// motor and vehicle params
	conf->loop_period_ms=LOOP_PERIOD_MS;
	conf->n_poles=N_POLES;
	conf->kt=KT;
	conf->r_wheel=R_WHEEL;
	conf->rho=RHO;
	conf->d_interwheel=D;

	// LQR controller params
	conf->gainsLQR[0]=CTL_BALANCE_XB_GAIN;
	conf->gainsLQR[1]=CTL_BALANCE_THETA_P_GAIN;
	conf->gainsLQR[2]=CTL_BALANCE_DOT_XB_GAIN;
	conf->gainsLQR[3]=CTL_BALANCE_DOT_THETA_P_GAIN;

	// velocity calculators
	filter_init_second_order_filter(&conf->dot_Xb_filt,HPF_COEF_N0,HPF_COEF_N1,HPF_COEF_N2,HPF_COEF_D1,HPF_COEF_D2);
	filter_init_second_order_filter(&conf->dot_delta_filt,HPF_COEF_N0,HPF_COEF_N1,HPF_COEF_N2,HPF_COEF_D1,HPF_COEF_D2);
	// acceleration calculator
	filter_init_second_order_filter(&conf->ddot_Xb_filt,HPF_COEF_N0,HPF_COEF_N1,HPF_COEF_N2,HPF_COEF_D1,HPF_COEF_D2);
	// integrators
	filter_init_first_order_filter(&conf->set_delta_filt,INT_COEF_N0,INT_COEF_N1,INT_COEF_D1);
	filter_init_first_order_filter(&conf->set_Xb_filt,INT_COEF_N0,INT_COEF_N1,INT_COEF_D1);
	filter_init_first_order_filter(&conf->set_dot_Xb_filt,INT_COEF_N0,INT_COEF_N1,INT_COEF_D1);

	// PID
	filter_init_first_order_filter(&conf->steering_PID,PID_COEF_N0,PID_COEF_N1,PID_COEF_D1);

	// accelerometer LPF
	filter_init_second_order_filter(&conf->accelero_filter,LPF_COEF_N0,LPF_COEF_N1,LPF_COEF_N2,LPF_COEF_D1,LPF_COEF_D2);

	conf->compl_filt_alpha = COMPLEM_FILT_ALPHA;
	conf->compl_filt_beta = COMPLEM_FILT_BETA;

	conf->gainsLQRsteering[0]=LQR_STEER_DELTA_GAIN;
	conf->gainsLQRsteering[1]=LQR_STEER_DOT_DELTA_GAIN;

	conf->imu_accx_offset=IMU_ACCX_OFFSET;
	conf->imu_accz_offset=IMU_ACCZ_OFFSET;
	conf->imu_gyro_offset=IMU_GYRO_OFFSET;

	conf->disturbance_torque_ampl=DIST_TORQUE_AMPL;
	conf->disturbance_torque_omega=DIST_TORQUE_OMEGA;
	filter_init_second_order_filter(&conf->gyro_filter,LPF_COEF_N0,LPF_COEF_N1,LPF_COEF_N2,LPF_COEF_D1,LPF_COEF_D2);

	// torque filters
	filter_init_second_order_filter(&conf->torque_bal_filt,NOTCH_COEF_N0,NOTCH_COEF_N1,NOTCH_COEF_N2,NOTCH_COEF_D1,NOTCH_COEF_D2);
	filter_init_second_order_filter(&conf->torque_steer_filt,NOTCH_COEF_N0,NOTCH_COEF_N1,NOTCH_COEF_N2,NOTCH_COEF_D1,NOTCH_COEF_D2);
}
...
/**
 * Read self-balancing configuration from EEPROM. If this fails, default values will be used.
 *
 * @param conf
 * A pointer to a SELF_BALANCING_PARAMS struct to write the read configuration to.
 */
void conf_general_read_self_balancing_params(SELF_BALANCING_PARAMS *conf) {
	bool is_ok = true;
	uint8_t *conf_addr = (uint8_t*)conf;
	uint16_t var;

	for (unsigned int i = 0;i < (sizeof(SELF_BALANCING_PARAMS) / 2);i++) {
		if (EE_ReadVariable(EEPROM_BASE_SELF_BAL_CONF + i, &var) == 0) {
			conf_addr[2 * i] = (var >> 8) & 0xFF;
			conf_addr[2 * i + 1] = var & 0xFF;
		} else {
			is_ok = false;
	...
	conf->dot_delta_filt.N0=conf->dot_Xb_filt.N0;
	conf->dot_delta_filt.N1=conf->dot_Xb_filt.N1;
	conf->dot_delta_filt.N2=conf->dot_Xb_filt.N2;
	conf->dot_delta_filt.D1=conf->dot_Xb_filt.D1;
	conf->dot_delta_filt.D2=conf->dot_Xb_filt.D2;

	conf->ddot_Xb_filt.N0=conf->dot_Xb_filt.N0;
	conf->ddot_Xb_filt.N1=conf->dot_Xb_filt.N1;
	conf->ddot_Xb_filt.N2=conf->dot_Xb_filt.N2;
	conf->ddot_Xb_filt.D1=conf->dot_Xb_filt.D1;
	conf->ddot_Xb_filt.D2=conf->dot_Xb_filt.D2;

	conf->gyro_filter.N0=conf->accelero_filter.N0;
	conf->gyro_filter.N1=conf->accelero_filter.N1;
	conf->gyro_filter.N2=conf->accelero_filter.N2;
	conf->gyro_filter.D1=conf->accelero_filter.D1;
	conf->gyro_filter.D2=conf->accelero_filter.D2;

	conf->torque_steer_filt.N0=conf->torque_bal_filt.N0;
	conf->torque_steer_filt.N1=conf->torque_bal_filt.N1;
	conf->torque_steer_filt.N2=conf->torque_bal_filt.N2;
	conf->torque_steer_filt.D1=conf->torque_bal_filt.D1;
	conf->torque_steer_filt.D2=conf->torque_bal_filt.D2;

	conf->set_delta_filt.N0=conf->set_Xb_filt.N0;
	conf->set_delta_filt.N1=conf->set_Xb_filt.N1;
	conf->set_delta_filt.D1=conf->set_Xb_filt.D1;

	conf->set_dot_Xb_filt.N0=conf->set_Xb_filt.N0;
	conf->set_dot_Xb_filt.N1=conf->set_Xb_filt.N1;
	conf->set_dot_Xb_filt.D1=conf->set_Xb_filt.D1;

	// Set the default configuration
	if (!is_ok) {
		conf_general_get_default_self_balancing_params(conf);
	}
}

/**
 * Write self-balancing configuration to EEPROM.
 *
 * @param conf
 * A pointer to the configuration that should be stored.
 */
bool conf_general_store_self_balancing_params(SELF_BALANCING_PARAMS *conf) {
	mc_interface_unlock();
	mc_interface_release_motor();
	...
	for (unsigned int i = 0;i < (sizeof(SELF_BALANCING_PARAMS) / 2);i++) {
		...
		if (EE_WriteVariable(EEPROM_BASE_SELF_BAL_CONF + i, var) != FLASH_COMPLETE) {
			is_ok = false;
			break;
		}
	}
